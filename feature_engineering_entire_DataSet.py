################################################################################################
##  Script Info: Extarcts features from headline and Body for the entire Dataset
...
def ExtractFeatures(feat_fn, headlines, bodies, feature_file):
...
def Jaccard_Similarity(headlines, bodies):
...
def sentiment_feature(headlines, bodies):
    X=[]
...
def named_entity_feature(headlines, bodies):
...
def Misc_features(headlines, bodies):
    ...
    X = []
    for i, (headline, body) in tqdm(enumerate(zip(headlines, bodies))):
        X.append(count_matching_tokens(headline, body)
...
def BoW_feature( bodies,bag):
    X=[]
    for i, (headline, body) in tqdm(enumerate(zip( bodies))):
