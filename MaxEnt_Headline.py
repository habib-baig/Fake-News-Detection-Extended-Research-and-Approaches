...
class Maxent:
  ...
  def crossValidationSplits(self, trainDir):
        ...
        for fold in range(0, self.numFolds):
            split    = self.TrainSplit()
            count    = 0
            splitFile  = "removed_articles" + str(fold) + ".csv" 
            scoredFile = "scored_articles" + str(fold) + ".csv"
            scoredFile = open(scoredFile, 'w')
            with open(splitFile, 'w') as csvfile:
                data          = ['headline', 'body', 'fakeness'] 
                archive       = csv.DictWriter(csvfile,    fieldnames = data)
                archive_score = csv.DictWriter(scoredFile, fieldnames = data)
                archive.writeheader()
                archive_score.writeheader()
                with open(trainDir) as fileName:
                    ...
                    for index,row in reader.iterrows():
                        ...
                        count    = count + 1
                        example = self.Example()
                        example.words = headline.split()
                        example.klass = polarity
                        if hashing != "hash" and hashing % 10 == fold:
                            split.test.append(example)
                            archive.writerow({'headline' : headline, 
                                              'body' : content, 
                                              'fakeness' : polarity})
                        else:
                            split.train.append(example)
                            archive_score.writerow({'headline' : headline, 
                                                    'body' : content, 
                                                    'fakeness' : polarity})
                csvfile.close()
                splits.append(split)
